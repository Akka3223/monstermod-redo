...
void CMISlave :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		{
			// SOUND HERE!
			edict_t *pHurt = CheckTraceHullAttack( 70, RANDOM_LONG(80, 90), DMG_SLASH );
			if ( pHurt )
			{
...
void CMISlave :: ZapBeam( int side )
{
	...
	if (pEntity != NULL && pEntity->v.takedamage)
	{
		if (UTIL_IsPlayer(pEntity))
			UTIL_TraceAttack( pEntity, pev, RANDOM_FLOAT(160, 170), vecAim, &tr, DMG_SHOCK );
		else if (pEntity->v.euser4 != NULL)
		{
			CMBaseMonster *pMonster = GetClassPtr((CMBaseMonster *)VARS(pEntity));
			pMonster->TraceAttack( pev, RANDOM_FLOAT(160, 170), vecAim, &tr, DMG_SHOCK );
		}
		else
			UTIL_TraceAttack( pEntity, pev, RANDOM_FLOAT(160, 170), vecAim, &tr, DMG_SHOCK );
	}
