	...
	if ((UTIL_Center(m_hEnemy) - pev->origin).Length() < 64)
	{
		...
		if (tr.pHit != NULL && tr.pHit->v.takedamage)
		{
			...
			if (UTIL_IsPlayer(tr.pHit))
				UTIL_TraceAttack( tr.pHit, VARS(m_hOwner), RANDOM_FLOAT(25.0, 30.0), pev->velocity, &tr, DMG_SHOCK );
			else if (tr.pHit->v.euser4 != NULL)
			{
				CMBaseMonster *pMonster = GetClassPtr((CMBaseMonster *)VARS(tr.pHit));
				pMonster->TraceAttack( VARS(m_hOwner), RANDOM_FLOAT(25.0, 30.0), pev->velocity, &tr, DMG_SHOCK );
			}
			else
				UTIL_TraceAttack( tr.pHit, VARS(m_hOwner), RANDOM_FLOAT(25.0, 30.0), pev->velocity, &tr, DMG_SHOCK );
...
void CMControllerZapBall::ExplodeTouch( edict_t *pOther )
{
	if (pOther->v.takedamage)
	{
		...
		if (UTIL_IsPlayer(pOther))
			UTIL_TraceAttack(pOther, pevOwner, RANDOM_FLOAT(20.0, 25.0), pev->velocity.Normalize(), &tr, DMG_ENERGYBEAM ); 
		else if (pOther->v.euser4 != NULL)
		{
			CMBaseMonster *pMonster = GetClassPtr((CMBaseMonster *)VARS(pOther));
			pMonster->TraceAttack(pevOwner, RANDOM_FLOAT(20.0, 25.0), pev->velocity.Normalize(), &tr, DMG_ENERGYBEAM ); 
		}
		else
			UTIL_TraceAttack(pOther, pevOwner, RANDOM_FLOAT(20.0, 25.0), pev->velocity.Normalize(), &tr, DMG_ENERGYBEAM );
